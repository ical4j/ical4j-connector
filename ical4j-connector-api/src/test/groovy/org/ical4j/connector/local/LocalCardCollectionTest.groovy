...
class LocalCardCollectionTest extends AbstractLocalTest {
    ...
    def 'test add card to collection'() {
        ...
        new File(workspaceLocation, "contacts/${card.uid.value}.vcf").exists()
    ...
    def 'test remove card from collection'() {
        ...
        def removed = collection.removeAll(card.uid.value)

        then: 'the existing card file is deleted'
        !new File(storeLocation, "contacts/${card.uid.getValue()}.vcf").exists()
    ...
    def 'test get card from collection'() {
        ...
        when: 'the card is removed'
        def retrieved = collection.get(card.uid.value)
