...
Read/write access to a user's Google Calendar through the iCal4j `ObjectStore` / `CalendarCollection` abstractions, including event ↔ iCal mapping for the common cases (summary, description, location, start/end with timezone or all-day, organizer, attendees, recurrence, created/updated, iCalUID).
